...
function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 rounded-b-lg bg-gradient-to-r from-gray-800 to-gray-900 p-6 flex flex-col md:flex-row items-center justify-between shadow-lg">
      <div className="text-2xl font-semibold text-orange-400 tracking-wide font-mono cursor-pointer">
      ...
      <nav className={`transition-all duration-300 ease-in-out overflow-hidden flex flex-col md:flex-row md:space-x-6 mt-4 md:mt-0 ${isOpen ? 'max-h-screen' : 'max-h-0 md:max-h-screen'}`}>
        <ul className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-8 font-mono text-lg">
          <li className="text-white hover:text-orange-400 transition duration-200 cursor-pointer">
            <Link to="home" smooth={true} duration={500}>
              Home
            </Link>
          </li>
